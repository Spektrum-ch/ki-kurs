...
export default function LektionBueroPage() {
  ...
  const prev = findPrevLessonInCourse(COURSE_BUERO, moduleSlug, lessonSlug);
  ...
  let lessonNumber = 0;
  let lessonCount = 0;
  COURSE_BUERO.modules.forEach(m => {
    m.lessons.forEach(l => {
      lessonCount++;
  ...
  async function handleComplete() {
    ...
    setTimeout(() => {
      if (next) router.push(`/kurs-buero/${next.moduleSlug}/${next.slug}`);
      else router.push('/zertifikat?course=kurs-buero');
    }, 800);
  }
  ...
  if (!found) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px' }}>
        ...
        <Link href="/kurs-buero" style={{ color: '#0057a8' }}>← Zurück zur Übersicht</Link>
      </div>
    );
  }

  const { module: modul, lesson: lektion } = found;
  const moduleIdx = COURSE_BUERO.modules.findIndex(m => m.id === modul.id);
    ...
    <>
      ...
      <div className="lk-layout">
        ...
        <aside className="lk-sidebar">
          <Link href="/kurs-buero" className="lk-sidebar-back">
        ...
        <main className="lk-content">
          ...
          {completed ? (
            <div className="lk-done-bar">
              ...
              {next ? (
                <Link href={`/kurs-buero/${next.moduleSlug}/${next.slug}`} className="lk-done-next">
                  Nächste Lektion
                ...
                <Link href="/zertifikat?course=kurs-buero" className="lk-done-next">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="lk-nav-bar">
              {prev ? (
                <Link href={`/kurs-buero/${prev.moduleSlug}/${prev.slug}`} className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Zurück
                ...
                <Link href="/kurs-buero" className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Übersicht
