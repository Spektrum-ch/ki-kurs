...
export default function LektionPage() {
  ...
  const next = findNextLesson(moduleSlug, lessonSlug);
  const prev = findPrevLesson(moduleSlug, lessonSlug);
  ...
  async function handleComplete() {
    ...
    setTimeout(() => {
      if (next) router.push(`/kurs/${next.moduleSlug}/${next.slug}`);
  ...
  if (!found) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px' }}>
        ...
        <Link href="/kurs" style={{ color: '#0057a8' }}>← Zurück zur Übersicht</Link>
      </div>
    );
  }

  const { module: modul, lesson: lektion } = found;
  const moduleIdx = COURSE.modules.findIndex(m => m.id === modul.id);
    ...
    <>
      ...
      <div className="lk-layout">
        ...
        <aside className="lk-sidebar">
          <Link href="/kurs" className="lk-sidebar-back">
        ...
        <main className="lk-content">
          ...
          {completed ? (
            <div className="lk-done-bar">
              ...
              {next ? (
                <Link href={`/kurs/${next.moduleSlug}/${next.slug}`} className="lk-done-next">
                  Nächste Lektion
            ...
            <div className="lk-nav-bar">
              {prev ? (
                <Link href={`/kurs/${prev.moduleSlug}/${prev.slug}`} className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Zurück
                ...
                <Link href="/kurs" className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Übersicht
