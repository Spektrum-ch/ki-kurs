...
export default function LektionAllgemeinPage() {
  const params = useParams();
  const router = useRouter();
  const moduleSlug = params.modul as string;
  const lessonSlug = params.lektion as string;
  const access = useCourseAccess('kurs-allgemein');

  const found = findLessonInCourse(COURSE_ALLGEMEIN, moduleSlug, lessonSlug);
  const next = findNextLessonInCourse(COURSE_ALLGEMEIN, moduleSlug, lessonSlug);
  const prev = findPrevLessonInCourse(COURSE_ALLGEMEIN, moduleSlug, lessonSlug);
  ...
  let lessonNumber = 0;
  let lessonCount = 0;
  COURSE_ALLGEMEIN.modules.forEach(m => {
    m.lessons.forEach(l => {
      lessonCount++;
  ...
  if (!found) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px' }}>
        ...
        <Link href="/kurs-allgemein" style={{ color: '#0057a8' }}>← Zurück zur Übersicht</Link>
      </div>
    );
  }

  const { module: modul, lesson: lektion } = found;
  const moduleIdx = COURSE_ALLGEMEIN.modules.findIndex(m => m.id === modul.id);
    ...
    <>
      ...
      <div className="lk-layout">
        ...
        <aside className="lk-sidebar">
          <Link href="/kurs-allgemein" className="lk-sidebar-back">
          ...
          {COURSE_ALLGEMEIN.modules.map((m, mi) => (
            <div key={m.id}>
              ...
              {m.lessons.map(l => {
                ...
                return (
                  <Link
                    key={l.id}
                    href={`/kurs-allgemein/${m.slug}/${l.slug}`}
        ...
        <main className="lk-content">
          ...
          {completed ? (
            <div className="lk-done-bar">
              ...
              {next ? (
                <Link href={`/kurs-allgemein/${next.moduleSlug}/${next.slug}`} className="lk-done-next">
                  Nächste Lektion
            ...
            <div className="lk-nav-bar">
              {prev ? (
                <Link href={`/kurs-allgemein/${prev.moduleSlug}/${prev.slug}`} className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Zurück
                ...
                <Link href="/kurs-allgemein" className="lk-nav-prev">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 12H5M12 5l-7 7 7 7"/>
                  </svg>
                  Übersicht
