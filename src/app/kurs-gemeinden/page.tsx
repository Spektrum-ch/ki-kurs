...
export const metadata: Metadata = {
  title: 'KI für Gemeinden – KI-Schulung für Gemeindeverwaltungen | SPEKTRUM',
  description: 'Praxisnahe KI-Schulung für Mitarbeitende in Gemeindeverwaltungen in der Schweiz, Deutschland und Österreich. KI in Verwaltung, Kommunikation und Raumplanung. 90 Tage Zugang – CHF 29.',
  keywords: ['KI Gemeindeverwaltung', 'KI Gemeinden Schweiz', 'KI Verwaltung Deutschland Österreich', 'Künstliche Intelligenz öffentliche Verwaltung', 'KI Schulung Gemeinde', 'KI Kurs Verwaltung DACH', 'ChatGPT Gemeindeverwaltung'],
  alternates: { canonical: 'https://kurse.spekt.ch/kurs-gemeinden' },
  openGraph: {
    title: 'KI für Gemeinden – Schulung für Gemeindeverwaltungen',
    description: 'Wie Gemeinden KI in Verwaltung, Kommunikation und Raumplanung einsetzen – für CH, DE und AT.',
    url: 'https://kurse.spekt.ch/kurs-gemeinden',
...
export default async function KursGemeindenPage() {
  ...
  const purchased = hasCourseAccess(user, 'kurs-gemeinden');
  const expiry = getCourseExpiry(user, 'kurs-gemeinden');
  const progress = auth ? getProgress(auth.email) : { completedLessons: [] as string[], quizResults: [], lastActivity: '' };
  const allLessonIds = COURSE_GEMEINDEN.modules.flatMap(m => m.lessons.map(l => `${m.slug}/${l.slug}`));
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        '@type': 'Course',
        name: 'KI für Gemeinden',
        description: 'Praxisnahe KI-Schulung für Mitarbeitende in Gemeindeverwaltungen. KI in Verwaltung, Kommunikation und Raumplanung.',
        url: 'https://kurse.spekt.ch/kurs-gemeinden',
        ...
        offers: { '@type': 'Offer', price: '29', priceCurrency: 'CHF', availability: 'https://schema.org/InStock', validFrom: '2024-01-01' },
        educationalLevel: 'Fortgeschritten',
        inLanguage: 'de',
        availableLanguage: 'de',
        coursePrerequisites: 'Grundkenntnisse KI von Vorteil',
        teaches: ['KI in der Gemeindeverwaltung', 'ChatGPT für öffentliche Verwaltung', 'KI-Kommunikation Gemeinden', 'KI Raumplanung Gemeinden'],
        hasCourseInstance: { '@type': 'CourseInstance', courseMode: 'online', courseWorkload: 'PT6H', inLanguage: 'de' },
      }) }} />
      ...
      <div className="kp-wrap">
        ...
        <div className="kp-header">
          <div className="kp-title-row">
            <h1 className="kp-title">{COURSE_GEMEINDEN.title}</h1>
            <span className="kp-badge">Fortgeschritten</span>
          </div>
          <p className="kp-subtitle">{COURSE_GEMEINDEN.description}</p>
        </div>
        ...
        {purchased ? (
          ...
          <div className="kp-price-card">
            <div className="kp-price-left">
              <div className="kp-price-amount">CHF 29</div>
              <div className="kp-price-sub">90 Tage Zugang</div>
            </div>
            <BuyButton courseSlug="kurs-gemeinden" label="Zugang kaufen – CHF 29" color="#00a896" />
          </div>
        )}
        ...
        {COURSE_GEMEINDEN.modules.map((modul, mi) => {
              ...
              <div className="kp-lessons">
                {modul.lessons.map((lektion, li) => {
                  ...
                  return (
                    <Link
                      key={lektion.id}
                      href={`/kurs-gemeinden/${modul.slug}/${lektion.slug}`}
        ...
        <div className="kp-final-card">
          ...
          <Link href="/zertifikat?course=kurs-gemeinden" className="kp-cert-badge" style={{ textDecoration: 'none', cursor: 'pointer' }}>
        ...
        {!purchased && (
          <div className="kp-cta-bar">
            <span className="kp-cta-text">
              90 Tage Zugang zu allen {COURSE_GEMEINDEN.modules.length} Modulen inkl. Abschlusszertifikat.
            </span>
            <BuyButton courseSlug="kurs-gemeinden" label="Zugang kaufen – CHF 29" color="#00a896" />
          </div>
        )}
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: '0 24px 48px', color: '#6e6e73', fontSize: 14, lineHeight: 1.7 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: '#1d1d1f', marginBottom: 10 }}>KI in der Gemeindeverwaltung – Schulung für Schweiz, Deutschland und Österreich</h2>
        <p>Gemeindeverwaltungen stehen vor der Frage: Wie nutzen wir Künstliche Intelligenz verantwortungsvoll und effektiv? Dieser Kurs gibt Mitarbeitenden in Gemeinden der Schweiz, Deutschlands und Österreichs konkrete Antworten. Von ChatGPT in der Bürgerkommunikation über KI in der Raumplanung bis zu automatisierten Verwaltungsprozessen. Du lernst, welche KI-Tools sich für die öffentliche Hand eignen, welche Risiken bestehen und wie du KI sicher und transparent einsetzt. Praxisnah, direkt anwendbar.</p>
      </div>
